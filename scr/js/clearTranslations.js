...
/***/ "./src/app/utils/createCtaButtonBar.ts":
/*!*********************************************!*\
  !*** ./src/app/utils/createCtaButtonBar.ts ***!
  \*********************************************/
...
exports.createCtaButtonBar = void 0;
...
function createCtaButtonBar() {
    ...
    loginButton.style.fontFamily = 'PatrickHand-Regular';
    ...
    loginButton.onclick = () => __awaiter(this, void 0, void 0, function* () {
        async_1.postBackgroundMessage({ kind: 'openLoginPopup' });
    });
    ...
    upgradebutton.style.fontFamily = 'PatrickHand-Regular';
    ...
    upgradebutton.onclick = () => {
        window.open('https://gumroad.com/l/ichigo-reader', '_blank');
    };
...
/***/ "./src/app/utils/createDomOverlay.ts":
/*!*******************************************!*\
  !*** ./src/app/utils/createDomOverlay.ts ***!
  \*******************************************/
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.createDomOverlay = exports.ichigoReaderElementClassName = void 0;
const $ = __webpack_require__(/*! jquery */ "./node_modules/jquery/dist/jquery.js");
const fitText_1 = __webpack_require__(/*! ./fitText */ "./src/app/utils/fitText.ts");
const createCtaButtonBar_1 = __webpack_require__(/*! ./createCtaButtonBar */ "./src/app/utils/createCtaButtonBar.ts");
const maxZindex = '2147483647';
exports.ichigoReaderElementClassName = 'ichigoReaderElement';
/**
 * Only use `createDomOverlay` if `createShadowDomOverlay` is not working.
 * `createShadowDomOverlay` is more robust in general, but fails on some sites such as pixiv.com
 * However, `createDomOverlay` fails on manhuagui.com and possibly others.
 * TODO: Look into unifying `createDomOverlay` and `createShadowDomOverlay` into a comprehensive single implementation.
 */
function createDomOverlay(element, onRemoved) {
    var _a, _b;
    if (element == null) {
        return null;
    }
    ...
    try {
        (_a = element === null || element === void 0 ? void 0 : element.parentNode) === null || _a === void 0 ? void 0 : _a.insertBefore(overlay, element);
    }
    catch (_c) {
        // Do nothing.
    }
    ...
    mutationObserver.observe((_b = document.body) !== null && _b !== void 0 ? _b : document.head, config);
    // Remove overlay if element src changes.
    const imageObserver = new MutationObserver(changes => {
    ...
    imageObserver.observe(element, { attributes: true });
    ...
    overlay.addText = (text, top, left, width, height, fontColor, fontFamily, isOutOfTranslations /** HACK: Special state for out of translations. */, backgroundImage) => {
        ...
        if (backgroundImage) {
            textBox.style.backgroundImage = `url(${backgroundImage})`;
        ...
        textBox.style.lineHeight = 'initial';
        ...
        if (isOutOfTranslations) {
            ...
            textBox.style.fontFamily = 'PatrickHand-Regular';
            textBox.style.color = '#976353';
            textBox.textContent = 'Feed the server hamsters.';
            textBox.append(createCtaButtonBar_1.createCtaButtonBar());
        }
        else {
        ...
        const [_, updateTextSizes] = fitText_1.fitText(textBox, isOutOfTranslations ? 0.26 : getFitTextScalar(fontFamily));
        overlayTextListeners.push(updateTextSizes);
    };
    return overlay;
}
exports.createDomOverlay = createDomOverlay;
function getFitTextScalar(fontFamily) {
    // All found by tweaking numbers until the text properly filled up the container.
    if (fontFamily === 'Kalam') {
        return 0.35;
    }
    ...
    console.warn(`Unsupported font family: ${fontFamily}`);
    return 0.3;
}
...
exports.fitText = function (el, kompressor) {
    const fit = function (el) {
        ...
        const resizer = function () {
            ...
            let tries = 0;
            const maxTries = 500;
            el.style.fontSize = el.clientWidth / (compressor * 10) + 'px';
            while (checkIsOverflowing(el)) {
                const radix = 10;
                const fontSize = parseInt(el.style.fontSize, radix);
                // Better to overflow than have an ultra tiny font size.
                const isBelowMinimum = fontSize < 10;
                if (isBelowMinimum) {
                    break;
                }
                const rescaled = fontSize - 1;
                el.style.fontSize = rescaled + 'px';
                tries++;
                if (tries > maxTries)
                    break;
            }
    ...
    if (el.length)
        ...
        fit(el);
    // HACK: Fixes an issue in Chrome where the fit wasn't perfect after rendering in the dom.
    // The number of 100 was found experimentally, but may need to be increased.
    // 50 was tried, but found to be too low in some cases.
    const postRenderTimeoutMs = 100;
    setTimeout(() => fit(el), postRenderTimeoutMs);
...
const createDomOverlay_1 = __webpack_require__(/*! ./utils/createDomOverlay */ "./src/app/utils/createDomOverlay.ts");
for (const shadowRoot of document.getElementsByClassName(shadowDomUtils_1.shadowDomRootClassName)) {
    shadowRoot.remove();
}
// HACK: Must wrap in a while loop because observers can continuously regenerate the elements.
while (document.getElementsByClassName(createDomOverlay_1.ichigoReaderElementClassName).length !== 0) {
    for (const overlay of document.getElementsByClassName(createDomOverlay_1.ichigoReaderElementClassName)) {
        overlay.remove();
    }
}
